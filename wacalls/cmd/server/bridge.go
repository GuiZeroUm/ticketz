...
func NewBridge(offerSDP string, log *slog.Logger) (*Bridge, string, error) {
	...
	if publicIP := strings.TrimSpace(os.Getenv("WACALLS_PUBLIC_IP")); publicIP != "" {
		settingEngine.SetNAT1To1IPs([]string{publicIP}, webrtc.ICECandidateTypeHost)
	}
	if portRange := strings.Split(strings.TrimSpace(os.Getenv("WACALLS_UDP_PORT_RANGE")), "-"); len(portRange) == 2 {
		minPort, minErr := strconv.ParseUint(portRange[0], 10, 16)
		maxPort, maxErr := strconv.ParseUint(portRange[1], 10, 16)
		if minErr == nil && maxErr == nil && minPort <= maxPort {
